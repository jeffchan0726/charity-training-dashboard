...
│   ├── food-nutrition.js      # 配對、份量、炒餸加油、USDA 後備
...
流程：**Grok 認餸** → **本地香港食物成分表第 6 版**（中菜優先）→ 未匹配再查 **USDA API（免費）** → 炒／煎餸預設加 1–1.5 鑊湯匙油（約 240–360 kcal）。

影相：約 45° 頂視，筷子或碗口入鏡做尺。冇尺可選 MiDaS 深度輔助（實驗）。

1. 將最新 `Google_Apps_Script.txt` 貼去 Apps Script，**新增一個部署版本**。
2. 指令碼屬性：
   - `XAI_API_KEY`：Grok 認餸（必須，[console.x.ai](https://console.x.ai)）
